...
export async function build({ top, page, ensure, doc, p, h, text, bold, italic, code, ul, ol, li, panel, table, picture, pageLink, adminAt }) {
  ...
  await page('Users', root, doc(
    ...
    ul(
      ...
      li(p(b('Actions:'), ' what you can do to the account. Only the actions that apply are shown. The owner’s row has none, and another administrator’s has none unless you are the owner or hold ', b('Manage administrators’ accounts'), ' (see ', pageLink('Roles'), ').')),
    ),
    ...
    p('Nobody can turn off the owner’s two-factor from here; the owner does it from their own profile. Another administrator’s can be turned off by the owner, or by someone the owner has given ', b('Manage administrators’ accounts'), '; your own, from your profile.'),
  ...
  await page('Roles', root, doc(
    ...
    ul(
      ...
      li(p(b('People:'), ' See the user list (which is also what shows other people’s email addresses), Manage accounts, Manage administrators’ accounts, Assign roles, Promote users to administrator, Manage invite links, Manage groups.')),
      li(p(b('Spaces:'), ' Manage spaces, Manage instance-wide templates, Publish spaces, Delete spaces, Control a space’s exports.')),
    ...
    p('Each row on the tab has a line saying what the right allows. Out of the box, users cannot delete other people’s pages or create invite links; administrators hold every right except Promote users to administrator, Manage administrators’ accounts, Restore a backup and Change the branding, which start with the owner.'),
    panel('info', p(b('Letting administrators recover each other.'), ' Out of the box, only the owner can sign out, suspend, reset the password of, or turn off two-factor for another administrator, because a password-reset link is a way into their account. If you, the owner, will not always be around, give the administrator role ', b('Manage administrators’ accounts'), ' so they can help each other. Nobody but the owner can ever do these things to the owner’s account.')),
