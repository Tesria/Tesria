...
export async function build({ top, page, ensure, doc, p, h, text, bold, italic, code, ul, ol, li, panel, table, live, picture, pageLink }) {
  ...
  await page('API tokens', profile, doc(
    ...
    step(3, 'Choose Create token, and copy it'),
    ...
    ...(await picture(tokens, 'api-tokens', 'Making an API token', 'A name, Read-only if it only reads, then Create token.')),
    ...
    ul(
      ...
      li(p(b('A token can do anything you can,'), ' unless it is read-only. It stops working if your role loses the right to use tokens.')),
      li(p(b('Tokens do not expire.'), ' The list shows when each was last used; ', b('Revoke'), ' one you no longer need. Anything using it stops working at once.')),
    ),
